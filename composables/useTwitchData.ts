...
const fetchStreamsWithProfiles = async (
  api: TwitchAPI,
  ...
  const usersResult = await api.fetch<{ data: any[] }>("users", {
    id: userIds,
  });
...
export const useTwitchData = <
  T,
  P extends Record<string, any> = { user_login?: string }
>(
  endpoint: Endpoint,
  initialParams: P,
  options: {
  ...
  const key = computed(
    () => `twitch-${endpoint}-${componentId}-${JSON.stringify(params)}`
  );
  ...
  const { data, error, pending, refresh } = useAsyncData<TwitchResponse<T>>(
    ...
    async () => {
      try {
        const result =
          endpoint === "streams" && options.includeUserProfiles
